...
- **`LANG=en_US.UTF-8` is required for a direct `pod install`.** CocoaPods
  normalises the project path through Ruby's Unicode tables and dies with
  `Encoding::CompatibilityError` under the default `ASCII-8BIT` locale.
  ⚠️ This was originally written up as a consequence of spaces in the path. It
  is not — it reproduced unchanged after the 2026-08-06 rename to a space-free
  path. It is purely the locale, and Homebrew prints the same advice on install.
  `expo run:ios` does **not** need it, because it only re-invokes CocoaPods when
  the Pods are stale.
...
#### ✅ Resolved 2026-08-06 — the checkout no longer has spaces in its path

**Keep the checkout at a path without spaces.** It used to live at
`/Users/…/New Matrimony App/…`; it is now
`/Users/…/New-Matrimony-App/zomyra/zomyra-app`. This section is kept because the
failure is worth recognising if anyone clones into `~/My Projects/` — and because
one of its three symptoms is silent.

An unquoted `$VAR` holding a path with spaces is word-split by the shell before
the command runs, so Xcode's `$PROJECT_DIR` / `$PODS_TARGET_SRCROOT` arrived as
three arguments. Three scripts in the iOS toolchain did that, none of them ours,
all of them regenerated by `expo prebuild` / `pod install` / `yarn install`:
...
| `ios/Zomyra.xcodeproj` — `Bundle React Native code and images` | same, at bundling time |
...
All three hand-patches have been **reverted to upstream** and the space-free path
verified against pristine scripts: the build is green, `EXConstants.bundle/app.config`
is generated (1262 bytes — under the bug it was absent), and both Maestro flows
pass. Nothing local is left to re-apply after a `prebuild` or `yarn install`.
