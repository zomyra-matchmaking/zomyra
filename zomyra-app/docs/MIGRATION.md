...
| C-7 | **Every module appends to the regression sheet** | Before closing a module, append its cases to `zomyra-app/docs/TEST-CASES.md` — IDs are `M<module>-<nnn>`, never reused, one assertion each, testing behaviour rather than implementation. The sheet exists so a later module that breaks an earlier one is caught **by name and by owning module**, runnable in its own session. **It is gitignored by owner decision** — so it is absent from PRs and from a fresh clone, and lives untracked on one machine. Nothing imports it, so Metro never bundles it either way (FE TDD §13.3); the ignore is repo hygiene, not bundle size. To start tracking it, drop the entry from `.gitignore`. |
